...
test('read and parse good.feature file', async () => {
  const result  = await featureProcessor.getGherkinDocument('./testdata/good.feature');
  expect(result.feature.name).toBe('Formula Tool Variables');
});

test('error: parse inexistent feature file', async () => {
  const result  = await featureProcessor.getGherkinDocument('./testdata/nope.feature');
  expect(result).toBe(undefined);
});

test('find all feature files recursively', async () => {
    const result = await featureProcessor.findAllFeatureFiles('/Users/EliseLebeau/git/seequcumber/testdata/someDirectory');
    expect(result.length).toBe(2);
});

test('error: find files in an inexistent directory', async () => {
  const result = await featureProcessor.findAllFeatureFiles('/Users/EliseLebeau/git/seequcumber/nope');
